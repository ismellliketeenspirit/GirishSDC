...
const seed = () => {
  let productsS = [];
  let counter = 0;
  while (productsS.length < 200) {
    let temp = faker.fake("{{commerce.productName}}");
    if (productsS.indexOf(temp) === -1){
    ...
    // console.log('Productss length ', productsS.length);
  }
  console.log('Product array:', productsS)
  let products = productsS.filter((item, index) => productsS.indexOf(item) === index);
  for (let i in products) {
    fetch(
      `https://api.unsplash.com/search/photos/?query=${products[i]}&client_id=${key.key.accessKey}`,
      {
        method: 'GET',
        headers: {
